...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- RENDER WALLET TAB COMPLETO ---
  function renderWalletTab(persona) {
    const walletList = persona.wallet || [];
    document.getElementById("badge-wallet-count").textContent = walletList.length;

    const grid = document.getElementById("wallet-files-grid");
    if (walletList.length === 0) {
      grid.innerHTML = `<p class="text-xs text-slate-400 py-6 italic col-span-3 text-center">Nessun documento caricato nel wallet.</p>`;
      return;
    }

    grid.innerHTML = walletList.map(doc => {
      const hasDownload = !!doc.fileContent;
      return `
        <div class="p-4 rounded-xl border border-slate-200 bg-slate-50/50 hover:border-blue-300 transition flex flex-col justify-between space-y-3">
          <div class="flex items-start justify-between space-x-3">
            <div class="flex items-start space-x-3">
              <div class="w-10 h-10 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center font-bold shrink-0 font-heading">
                <i class="fa-solid fa-file-pdf text-lg"></i>
              </div>
              <div class="space-y-0.5">
                <h4 class="text-xs font-bold text-slate-800 line-clamp-1 font-heading" title="${escapeHtml(doc.nome)}">${escapeHtml(doc.nome)}</h4>
                <span class="inline-block text-[10px] font-semibold px-2 py-0.2 rounded bg-amber-50 text-amber-800 border border-amber-200">${escapeHtml(doc.tipo)}</span>
                <p class="text-[10px] text-slate-400 font-mono">Data: ${formatDate(doc.data)} | ${doc.dimensione}</p>
              </div>
            </div>

            <button data-doc-id="${doc.id}" class="btn-delete-doc cursor-pointer text-slate-400 hover:text-rose-600 text-xs p-1" title="Elimina allegato">
              <i class="fa-solid fa-trash"></i>
            </button>
          </div>

          ${doc.descrizione ? `<p class="text-[11px] text-slate-600 bg-white p-2 rounded-lg border border-slate-200 italic">${escapeHtml(doc.descrizione)}</p>` : ''}
          ...
          <div class="pt-2 border-t border-slate-200/80 flex items-center justify-between">
            ${hasDownload ? `
              <a href="${doc.fileContent}" download="${doc.nome}" class="cursor-pointer text-xs text-blue-600 font-semibold hover:underline flex items-center gap-1 font-heading">
                <i class="fa-solid fa-download"></i> Scarica File Reale
              </a>
            ` : `
              <span class="text-[10px] text-slate-400 italic"><i class="fa-solid fa-check text-emerald-600 mr-1"></i> Documento Archivio CPI</span>
            `}
  ...
  function renderAuditLogsTable() {
    ...
    const filtered = logs.filter(l => filterUser === "ALL" || l.operatore.includes(filterUser));
  ...
  // --- NUOVO ISCRITTO DIRETTO (SCHEDA 360° PULITA E GUIDATA) ---
  ...
  if (btnQuickNew) {
    btnQuickNew.addEventListener("click", () => {
      ...
      if (btnNuovo) {
        ...
        // Fallback: switch to hub and trigger new citizen
        document.getElementById("nav-mode-hub").click();
      }
    });
  }

  function openEditPersonaModal(isNew = false) {
    const p = isNew ? null : window.store.getSelectedPersona();
    document.getElementById("modal-persona-title").textContent = isNew ? "Nuovo Iscritto Collocamento Mirato" : "Modifica Anagrafica Iscritto";

    if (p && !isNew) {
      document.getElementById("m-nome").value = p.nome || "";
      document.getElementById("m-cf").value = p.codiceFiscale || "";
      document.getElementById("m-cat-select").value = p.categoria || "C.O.";
      document.getElementById("m-ic-perc").value = p.icPercentuale || 0;
      document.getElementById("m-residenza").value = p.comuneResidenza || "";
      document.getElementById("m-telefono").value = p.cellulare || p.telefono1 || p.telefono || "";
    } else {
      document.getElementById("m-nome").value = "";
      document.getElementById("m-cf").value = "";
      document.getElementById("m-cat-select").value = "C.O.";
      document.getElementById("m-ic-perc").value = 46;
      document.getElementById("m-residenza").value = "Lecco";
  ...
  document.getElementById("btn-save-modal-persona").addEventListener("click", () => {
    const nome = document.getElementById("m-nome").value.trim();
    const cf = document.getElementById("m-cf").value.trim().toUpperCase();

    if (!nome || !cf) {
      alert("Inserire Nome e Codice Fiscale.");
      return;
    }
    ...
    const payload = {
      nome,
      codiceFiscale: cf,
      categoria: document.getElementById("m-cat-select").value,
      icPercentuale: parseInt(document.getElementById("m-ic-perc").value) || 0,
      comuneResidenza: document.getElementById("m-residenza").value || "Lecco",
      telefono1: document.getElementById("m-telefono").value,
      attivoNonAttivo: "Attivo",
      stato: "Disoccupato",
      operatore: "CPI Lecco"
    };

    const currentP = window.store.getSelectedPersona();
    if (currentP && document.getElementById("modal-persona-title").textContent.includes("Modifica")) {
      window.store.updatePersona(currentP.id, payload);
    } else {
      const newP = window.store.addPersona(payload);
      window.store.setSelectedPersonaId(newP.id);
    }
